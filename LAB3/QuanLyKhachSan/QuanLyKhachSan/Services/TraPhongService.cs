...
namespace QuanLyKhachSan.Services
{
    public class TraPhongService
    {
        ...
        {
            return Db.Query(@"SELECT q.*, l.TenLoaiTN FROM QuyDinhDenBu q
        ...
        {
            return Db.Query(@"SELECT h.*, k.HoTen FROM HoaDon h
        ...
        public KetQuaXuLy LapHoaDon(string soHD, string soDat, DateTime ngay, string maNV, int soNgay)
        {
            ...
            try
            {
                ...
                Db.Execute(@"INSERT INTO HoaDon(SoHoaDon,SoPhieuDat,NgayLap,MaNV,SoNgayTinhTien,TienPhong,TienDichVu,TrangThai)
                    ...
                    new SqlParameter("@h", soHD), new SqlParameter("@s", soDat),
                    new SqlParameter("@n", ngay), new SqlParameter("@nv", maNV),
                    new SqlParameter("@ng", soNgay), new SqlParameter("@p", phong),
                    new SqlParameter("@d", dv));

                return KetQuaXuLy.Ok("Đã lập hóa đơn tiền phòng và dịch vụ. Số ngày tính tiền là dữ liệu nhân viên " +
                    "xác nhận vì đề gốc không quy định cách làm tròn ngày lưu trú.");
            }
        ...
        public KetQuaXuLy TraPhong(string soDat, DateTime ngayTra)
        {
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    var u1 = new SqlCommand(@"UPDATE PhieuDatPhong SET TrangThai=N'Đã trả',NgayTraThucTe=@n
