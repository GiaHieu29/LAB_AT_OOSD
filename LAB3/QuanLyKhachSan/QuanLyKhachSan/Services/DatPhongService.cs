...
namespace QuanLyKhachSan.Services
{
    public class DatPhongService
    {
        ...
        {
            return Db.Query(@"SELECT p.*, k.TenKhuVuc FROM Phong p
        ...
        {
            return Db.Query(@"SELECT d.*, k.HoTen FROM PhieuDatPhong d
        ...
        {
            return Db.Query(@"SELECT c.*, p.SoNguoiToiDa, p.DonGiaNgay
        ...
        public KetQuaXuLy ThemNguoiLuuTru(string so, string phong, string ten, string cmnd, string qt)
        {
            ...
            try
            {
                ...
                Db.Execute("INSERT INTO NguoiLuuTru(SoPhieuDat,SoPhong,HoTen,SoCMND,QuocTich) VALUES(@s,@p,@t,@c,@q)",
                    new SqlParameter("@s", so), new SqlParameter("@p", phong),
                    new SqlParameter("@t", ten), new SqlParameter("@c", cmnd),
        ...
        public KetQuaXuLy NhanPhong(string so, DateTime thucTe)
        {
            ...
            using (var tx = cn.BeginTransaction())
            {
                try
                {
                    var c = new SqlCommand(@"UPDATE PhieuDatPhong SET TrangThai=N'Đang ở',NgayNhanThucTe=@n
                    ...
                    var u = new SqlCommand(@"UPDATE Phong SET TrangThai=N'Đang ở'
        ...
        public KetQuaXuLy DanhDauNoShow(string so)
        {
            try
            {
                Db.Execute("UPDATE PhieuDatPhong SET TrangThai=N'No-show' WHERE SoPhieuDat=@s AND TrangThai=N'Đã đặt'",
                    new SqlParameter("@s", so));
                Db.Execute(@"UPDATE Phong SET TrangThai=N'Trống'
                             WHERE SoPhong IN(SELECT SoPhong FROM ChiTietDatPhong WHERE SoPhieuDat=@s)",
                    new SqlParameter("@s", so));
                return KetQuaXuLy.Ok("Đã đánh dấu không nhận phòng. Việc mất cọc được giữ theo quy định gốc; " +
                    "đề không nêu thời gian tự động nên thao tác này do nhân viên xác nhận.");
            }
            catch (Exception ex) { return KetQuaXuLy.Fail(ex.Message); }
        }
    }
