            ...
            string tinhTrang, decimal phiPhat)
        {
            ...
            using (SqlTransaction tx = cn.BeginTransaction(IsolationLevel.Serializable))
            {
                try
                {
                    ...
                    // Sách mất hoặc hư hỏng được coi là không còn sẵn sàng cho mượn.
                    if (!mat && !huHong)
                    {
                    ...
                    if (canPhat)
                    {
                        ...
                        using (SqlCommand phat = new SqlCommand(@"INSERT INTO
                            PhieuPhat(MaPhieuPhat,MaChiTiet,MaNhanVien,NgayPhat,LyDo,PhiPhat)
